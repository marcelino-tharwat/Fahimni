...
describe('validateQuestionDraft', () => {
  const mcq = { type: 'MCQ' as const, content: 'Q', options: ['a', 'b', 'c', 'd'], correctAnswer: 'a' };
  ...
  it('requires a TF correct answer to be صح/خطأ', () => {
    expect(validateQuestionDraft({ type: 'TRUE_FALSE', content: 'Q', options: ['صح', 'خطأ'], correctAnswer: null }).correctAnswer).toBeTruthy();
    expect(validateQuestionDraft({ type: 'TRUE_FALSE', content: 'Q', options: ['صح', 'خطأ'], correctAnswer: 'صح' })).toEqual({});
  });
  it('accepts an essay with only content', () => {
    expect(validateQuestionDraft({ type: 'ESSAY', content: 'Explain', options: [], correctAnswer: null })).toEqual({});
  });
});

describe('draftToApiPayload', () => {
  it('converts MCQ options to a record and keeps the correct answer', () => {
    const p = draftToApiPayload({ type: 'MCQ', content: ' Q ', options: ['a', 'b', 'c', 'd'], correctAnswer: 'b' });
  ...
  it('clears options/answer for essay', () => {
    const p = draftToApiPayload({ type: 'ESSAY', content: 'Explain', options: [], correctAnswer: null });
    expect(p.options).toEqual({});
    expect(p.correctAnswer).toBeNull();
  });
  it('forces TF options to صح/خطأ', () => {
    const p = draftToApiPayload({ type: 'TRUE_FALSE', content: 'Q', options: [], correctAnswer: 'صح' });
