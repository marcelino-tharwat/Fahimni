...
function ChemicalBackground() {
  return (
    ...
    >
      {/* Molecular cluster */}
      <svg
        className="absolute left-[6%] top-8 h-36 w-36 text-purple-400 opacity-[0.04]"
        viewBox="0 0 100 100"
        fill="none"
        stroke="currentColor"
        strokeWidth="1.2"
      ...
      {/* Hexagon structure */}
      <svg
        className="absolute right-[10%] top-4 h-40 w-40 text-purple-300 opacity-[0.04]"
        viewBox="0 0 100 100"
        fill="none"
        stroke="currentColor"
        strokeWidth="1.2"
      ...
      {/* Electron orbital */}
      <svg
        className="absolute left-[20%] top-24 h-24 w-24 text-cyan-400 opacity-[0.03]"
        viewBox="0 0 100 100"
        fill="none"
        stroke="currentColor"
        strokeWidth="1"
      ...
      <svg
        className="absolute right-[18%] top-28 h-16 w-20 text-cyan-300 opacity-[0.04]"
        viewBox="0 0 80 40"
        fill="none"
      >
        <text
      ...
      <svg
        className="absolute left-[38%] top-5 h-14 w-20 text-purple-300 opacity-[0.03]"
        viewBox="0 0 80 40"
        fill="none"
      >
        <text
    ...
    <>
      <nav className="absolute top-0 left-0 z-50 flex w-full items-center justify-between px-6 py-5 md:px-12 md:py-6">
        ...
        <a
          ...
          className="flex items-center gap-2 font-cairo text-lg font-bold text-white transition-opacity hover:opacity-80 md:text-xl"
        >
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="rounded-full border border-white/20 px-4 py-1.5 font-cairo text-xs font-medium text-white/80 transition-all duration-200 hover:border-white/40 hover:text-white md:text-sm"
          >
          ...
          <button
            ...
            className="rounded-btn bg-white px-5 py-2 font-cairo text-xs font-semibold text-navy-900 transition-all duration-200 hover:bg-gray-100 md:text-sm"
          >
            Login
          </button>
          ...
          <button
            type="button"
            onClick={() => navigate("/auth")}
            className="rounded-btn bg-cyan-500 px-5 py-2 font-cairo text-xs font-semibold text-navy-900 transition-all duration-200 hover:bg-cyan-400 md:text-sm"
          >
            {t("nav.subscribe")}
          </button>
      ...
      {/* Mobile dropdown */}
      {menuOpen && (
        <div className="absolute top-full left-0 z-50 w-full bg-navy-900/80 backdrop-blur-md px-6 py-4 md:hidden">
          <div className="flex flex-col gap-2">
            {navLinks.map((link) => (
              <button
                ...
                onClick={() => handleNav(link.id)}
                className={cn(
                  "w-full rounded-btn px-3 py-2 text-start font-cairo text-sm font-medium transition-colors hover:bg-white/10",
                  active === link.id
                    ? "text-cyan-400"
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <p className="mt-0.5 font-cairo text-xs leading-tight text-gray-400">
          {label}
        </p>
      </div>
    </div>
...
export function HeroSection() {
  ...
  return (
    <section className="relative h-dvh overflow-hidden bg-hero-gradient">
      <style>{`
        @keyframes float {
      ...
      <div className="relative z-10 mx-auto flex h-full max-w-7xl flex-col px-6">
        <div className="flex min-h-0 flex-1 flex-col items-center justify-center gap-6 lg:flex-row lg:gap-12">
          ...
          <div className="flex w-full flex-col items-center text-center lg:w-1/2 lg:items-start lg:text-start">
            <div className="mb-5 inline-flex items-center rounded-full bg-teal-500/10 px-3 py-1 text-xs font-semibold text-teal-400 font-cairo">
              Grade 12
            </div>

            <h1 className="mb-4 font-cairo text-3xl font-extrabold leading-tight md:text-5xl lg:text-6xl">
            ...
            <p className="mb-6 max-w-lg font-cairo text-sm leading-relaxed text-gray-400 md:text-base">
            ...
            <div className="flex flex-wrap gap-4">
              <button
                type="button"
                onClick={() => navigate("/auth")}
                className="rounded-btn bg-cyan-500 px-7 py-3 font-cairo text-sm font-bold text-navy-900 shadow-[0_0_20px_rgba(0,201,219,0.3)] transition-all duration-200 hover:bg-cyan-400 hover:shadow-[0_0_30px_rgba(0,201,219,0.5)] hover:scale-105"
              >
                Start Learning Now
              </button>
              <button
                type="button"
                onClick={() => navigate("/auth")}
                className="flex items-center gap-2 rounded-btn border border-white/20 bg-white/5 px-7 py-3 font-cairo text-sm font-semibold text-white backdrop-blur-sm transition-all duration-200 hover:border-white/40 hover:bg-white/10 hover:scale-105"
              >
          ...
          <div className="flex w-full items-center justify-center lg:w-1/2">
            <div className="relative w-full max-h-[50vh] lg:max-h-[60vh]">
              <img
                src="/images/hero.png"
